...
async def read_prompt(item_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to find prompt: {str(e)}"
        ) from e
...
@router.put(
    ...
    summary="Update Prompt Information",
)
async def update_prompt(
    item_id: int, item_in: PromptUpdate, db: AsyncSession = Depends(get_db)
):
    """
...
@router.delete("/{item_id}", summary="Delete Prompt")
async def delete_prompt(item_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        prompt = await prompt_crud.get(db=db, id=item_id)
        if not prompt:
            raise HTTPException(status_code=404, detail="Prompt not found")

        # Delete the prompt
        await prompt_crud.remove(db=db, id=item_id)
...
async def read_prompts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get prompt list: {str(e)}"
        ) from e
